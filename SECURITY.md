...
Responder is trusted with Slack application credentials, webhook secrets, and the local Coop
...
Responder and Coop use the same Unix account. Systemd hardening reduces accidental exposure but
...
- Grant Docker socket access only to the Coop process. The shipped systemd unit adds the `docker`
  supplementary group to Coop, not to Responder. Managed foreground mode necessarily gives the
