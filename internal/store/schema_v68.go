...
// schemaV68 extends the durable Slack outbox with reaction writes. SQLite
// cannot alter a CHECK constraint in place, so preserve every delivery column
// while rebuilding the table with the additional operation.
const schemaV68 = `
CREATE TABLE slack_deliveries_v68 (
  id TEXT PRIMARY KEY,
  incident_id TEXT,
  operation TEXT NOT NULL CHECK (operation IN ('post', 'update', 'status', 'file', 'reaction')),
  kind TEXT NOT NULL,
...
INSERT INTO slack_deliveries_v68 (
...
ALTER TABLE slack_deliveries_v68 RENAME TO slack_deliveries;
