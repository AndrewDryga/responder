...
func TestWatchPromptCarriesMandatoryCrossSourceEvidencePolicy(t *testing.T) {
	...
	for _, required := range []string{
		...
		"Finish with one practical overall verdict: healthy, degraded, or unhealthy",
		"A formal SLO is optional",
		"do not call the whole platform unknown",
		"no more than six evidence-rich bullets",
