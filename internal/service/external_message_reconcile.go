...
func (s *Service) reconcileExternalMessage(
	...
	var current slackui.HistoryMessage
	for _, message := range history {
		if message.Timestamp == source.MessageTS {
			current = message
			break
		}
	}
	...
	if created {
		...
		_ = s.store.Audit(ctx, core.AuditEvent{
			...
			ObjectID: updated.ID, Outcome: "reconciled_update", Detail: source.ID,
		})
	}
...
func reconciledExternalMessageInput(
	...
	return core.SlackInput{
		EnvelopeID: "reconcile:" + source.ID + ":" + fingerprint,
		EventID:    "reconcile:" + source.ID + ":" + fingerprint,
