...
- record_finding: {"id":"finding-1","type":"record_finding","finding":{"what":"one line naming the failure state","scope":"service or environment","status":"unexplained|explained|expected|out_of_scope","cause_evidence":["evidence id, required when explained"],"alternatives":[{"hypothesis":"strongest rival cause","discriminated_by":"evidence id"}],"reason":"required for expected|out_of_scope"}} — record one for EVERY failure state your reply mentions; unexplained is the honest default until evidence identifies the cause. An unexplained finding may close only with an alternative whose not_checkable says what check would settle it and why it is unavailable now.
